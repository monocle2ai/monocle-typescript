...
const OKAHU_PROD_INGEST_ENDPOINT =
...
export class OkahuSpanExporter implements SpanExporter {
  ...
  constructor(config: OkahuSpanExporterConfig = {}) {
    ...
    consoleLog(
      `OkahuSpanExporter| Initializing with endpoint: ${this.endpoint}, timeout: ${this.timeout}`,
    );
  ...
  export(
    ...
    if (this._closed) {
      consoleLog(
        "OkahuSpanExporter| Exporter already shutdown, ignoring batch",
      );
  ...
  private async _sendSpans(
    ...
    try {
      consoleLog(
        `OkahuSpanExporter| Sending batch to endpoint: ${this.endpoint}`,
      );
      const result = await this.client.post(this.endpoint, spanListLocal);
      consoleLog(
        `OkahuSpanExporter| Received response - Status: ${result.status}, Data: ${JSON.stringify(result.data)}`,
      );
      if (!REQUESTS_SUCCESS_STATUS_CODES.includes(result.status)) {
        console.error(
          `OkahuSpanExporter| Export failed - Status: ${result.status}, Response: ${JSON.stringify(result.data)}`,
        );
        if (resultCallback) {
          resultCallback({ code: ExportResultCode.FAILED });
        }
      ...
      consoleLog(
        `OkahuSpanExporter| Successfully exported ${spanListLocal.batch.length} spans`,
      );
