...
const langchainPackages = [
    {
        ...
        attributeSetter: ({ returnedValue, arguments, classInstance, span }) => {
            ...
            const llm_input = arguments[0] && arguments[0].value
            if (typeof llm_input == "string") {
                span.setAttribute(LLM_INPUT_KEY, llm_input)
            }

            const llm_output = returnedValue && returnedValue.content
            if (typeof llm_output == "string") {
                span.setAttribute(LLM_OUTPUT_KEY, llm_output)
            }
