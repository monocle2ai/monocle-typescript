...
describe('Bedrock Runtime Client Sample', () => {
    ...
    it('should generate all expected spans', async () => {
        ...
        const ndjsonPath = join(__dirname, '../examples/bedrockRuntimeClient.ndjson');
        const ndjsonContent = readFileSync(ndjsonPath, 'utf-8');
        const expectedSpans = ndjsonContent
            .split('\n')
            .filter(line => line.trim() && !line.trim().startsWith('Invoking')) // Filter out the "Invoking..." log
